...
class SimpleAttention(nn.Module):
    '''
    在 DialogueRNN 里，需要从历史全局状态 g_hist 里挑出“对当前句子最相关的上下文向量”c_。
    SimpleAttention 做的就是：对历史每个时刻给一个权重，然后加权求和得到 c_。
    '''
    ...
    def forward(self, M, x=None):
        ...
        """
        scale = self.scalar(M) # seq_len, batch, 1  给每个历史时刻打分
        alpha = F.softmax(scale, dim=0).permute(1,2,0) # batch, 1, seq_len  在时间维做softmax：得到权重
        attn_pool = torch.bmm(alpha, M.transpose(0,1))[:,0,:] # batch, vector  加权求和：得到 c

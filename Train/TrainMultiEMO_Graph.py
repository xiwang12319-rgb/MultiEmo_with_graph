...
class TrainMultiEMO():
    ...
    def __init__(self, dataset, batch_size, num_epochs, learning_rate, weight_decay,
                 ...
                 HGR_loss_param, CE_loss_param, multi_attn_flag, graph_type, device):
    ...
    def get_model(self):
        ...
        self.model = build_model_graph(self.dataset, self.multi_attn_flag, roberta_dim, hidden_dim, dropout_rate, num_layers,
                                       self.model_dim, num_heads, D_m_audio, D_m_visual, D_g, D_p, D_e,
                                       D_h, self.num_classes, self.n_speakers,
                                       listener_state, context_attention, D_a, self.dropout_rec, self.device,
                                       graph_type = self.graph_type).to(self.device)

    def get_loss(self):
        class_counts = self.get_class_counts()
        self.SWFC_loss = SampleWeightedFocalContrastiveLoss(self.temp_param, self.focus_param,
                                                            self.sample_weight_param, self.dataset, class_counts,
                                                            self.device)
    ...
    def get_optimizer(self):
        ...
        self.scheduler = optim.lr_scheduler.ReduceLROnPlateau(self.optimizer, factor = 0.95, patience = 10, threshold = 1e-6,
                                                              verbose = True)
    ...
    def train_or_eval_model_per_epoch(self, dataloader, train = True):
        ...
        for _, data in enumerate(dataloader):
            ...
            fused_text_features, fused_audio_features, fused_visual_features, fc_outputs, mlp_outputs = \
                self.model(padded_texts, padded_audios, padded_visuals, padded_speaker_masks, padded_utterance_masks,
                           padded_labels)
        ...
        return round(total_loss, 4), round(total_HGR_loss, 4), round(total_SWFC_loss, 4), round(total_CE_loss,
                                                                                                4), avg_f1, avg_acc, report


    def train_or_eval_linear_model(self):
        for e in range(self.num_epochs):
            train_loss, train_HGR_loss, train_SWFC_loss, train_CE_loss, train_f1, train_acc, _ = self.train_or_eval_model_per_epoch(
                self.train_dataloader, train = True)
            with torch.no_grad():
                valid_loss, valid_HGR_loss, valid_SWFC_loss, valid_CE_loss, valid_f1, valid_acc, _ = self.train_or_eval_model_per_epoch(
                    self.valid_dataloader, train = False)
                test_loss, test_HGR_loss, test_SWFC_loss, test_CE_loss, test_f1, test_acc, test_report = self.train_or_eval_model_per_epoch(
                    self.test_dataloader, train = False)
            print(
                'Epoch {}, train loss: {}, train HGR loss: {}, train SWFC loss: {}, train CE loss: {}, train f1: {}, train acc: {}'.format(
                    e + 1, train_loss, train_HGR_loss, train_SWFC_loss, train_CE_loss, train_f1, train_acc))
            print(
                'Epoch {}, valid loss: {}, valid HGR loss: {}, valid SWFC loss: {}, valid CE loss: {}, valid f1: {}, valid acc: {}'.format(
                    e + 1, valid_loss, valid_HGR_loss, valid_SWFC_loss, valid_CE_loss, valid_f1, valid_acc))
            print(
                'Epoch {}, test loss: {}, test HGR loss: {}, test SWFC loss: {}, test CE loss: {}, test f1: {}, test acc: {}, '.format(
                    e + 1, test_loss, test_HGR_loss, test_SWFC_loss, test_CE_loss, test_f1, test_acc))
            print(test_report)
...
def get_args():
    ...
    parser.add_option('--graph_type', dest = 'graph_type', type = 'choice', default = 'both',
                      choices = ['temporal', 'speaker', 'both', 'none'],
                      help = 'graph structure type: temporal, speaker, both, none')
...
if __name__ == '__main__':
    ...
    print('Graph mode: {}'.format(graph_type))
    ...
    multiemo_train = TrainMultiEMO(dataset, batch_size, num_epochs, learning_rate,
                                   ...
                                   SWFC_loss_param, HGR_loss_param, CE_loss_param, multi_attn_flag, graph_type, device)

...
class TrainMultiEMO():
    ...
    def __init__(self, dataset, batch_size, num_epochs, learning_rate, weight_decay,
                 ...
                 HGR_loss_param, CE_loss_param, multi_attn_flag, limit_train, limit_test, device):
    ...
    def get_dataloader(self, valid = 0.05):
        if self.dataset == 'IEMOCAP':
            ...
            base_train_dataset = MELDDataset(train = True)
            base_test_dataset = MELDDataset(train = False)

        train_dataset = base_train_dataset
        test_dataset = base_test_dataset
        if self.limit_train and self.limit_train > 0:
            limit = min(self.limit_train, len(base_train_dataset))
            train_dataset = Subset(base_train_dataset, list(range(limit)))
        if self.limit_test and self.limit_test > 0:
            limit = min(self.limit_test, len(base_test_dataset))
        ...
        self.train_dataloader = DataLoader(dataset = train_dataset, batch_size = self.batch_size,
                                           sampler = train_sampler, collate_fn = base_train_dataset.collate_fn, num_workers = 0)
        self.valid_dataloader = DataLoader(dataset = train_dataset, batch_size = self.batch_size,
                                          sampler = valid_sampler, collate_fn = base_train_dataset.collate_fn, num_workers = 0)
        self.test_dataloader = DataLoader(dataset = test_dataset, batch_size = self.batch_size,
                                          collate_fn = base_test_dataset.collate_fn, shuffle = False, num_workers = 0)
...
def get_args():
    ...
    parser.add_option('--limit_train', dest = 'limit_train', default = 0, type = 'int', help = 'limit train samples (0 = no limit)')
...
if __name__ == '__main__':
    ...
    multiemo_train = TrainMultiEMO(dataset, batch_size, num_epochs, learning_rate,
                                   ...
                                   SWFC_loss_param, HGR_loss_param, CE_loss_param, multi_attn_flag,
                                   limit_train, limit_test, device)
